...
NEXT_PUBLIC_API_URL="/api"
API_PROXY_TARGET="http://127.0.0.1:3001"
HEROUI_AUTH_TOKEN="你的 HeroUI Pro CI/CD Token"
```

说明：

- `Dockerfile.frontend` 从仓库根目录复制 `frontend/`，不需要设置 Root Directory。
- `NEXT_PUBLIC_API_URL` 推荐保持 `/api`，由 Next.js 服务端代理到后端。
- `API_PROXY_TARGET` 填写后端地址，不需要 `/api` 后缀；Zeabur 模板会自动使用内网地址 `http://${BACKEND_HOST}:8080`。
- `HEROUI_AUTH_TOKEN` 必须使用 HeroUI Pro 的 CI/CD Token，否则 `npm ci` 会在构建阶段失败。
...
1. 推荐使用根目录 `zeabur.yaml` 作为一键部署模板。
2. 模板会自动创建 `mysql`、`backend`、`frontend` 三个独立服务。
3. Backend / Frontend 都从仓库根目录构建，不需要你选择 `backend` 或 `frontend` 目录。
4. 模板内已明确内联 Dockerfile 内容，不再走 Static 自动探测。
5. 部署时需要填写 `FRONTEND_DOMAIN`、`BACKEND_DOMAIN` 和 `HEROUI_AUTH_TOKEN`。
6. 部署完成后打开 Frontend 域名创建第一个管理员账号。

注意：在 Zeabur 里普通导入 GitHub 仓库通常只会创建一个服务，并可能被识别成 Static。要自动创建三服务，必须走模板部署：使用 `zeabur.yaml` 发布为模板，或用 Zeabur CLI/控制台从模板 YAML 创建项目。

Zeabur 模板内的 Backend / Frontend HTTP 端口统一使用 `8080`，Frontend 会通过 `API_PROXY_TARGET=http://${BACKEND_HOST}:8080` 走 Zeabur 内网访问 Backend，避免公网域名尚未稳定时出现 API 500。
...
如果你在 Zeabur UI 里只看到一个 Docker 服务，说明你走的是单容器模式。这个服务必须同时设置：

```env
DATABASE_URL="完整 MySQL 连接串，不要写 ${MYSQL_CONNECTION_STRING}"
NEXT_PUBLIC_API_URL="/api"
API_PROXY_TARGET="http://127.0.0.1:3001"
AUTH_SECRET="生成一个足够长的随机字符串"
AUTH_TOKEN_TTL_DAYS=30
